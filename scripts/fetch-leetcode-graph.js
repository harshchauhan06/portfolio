...
function normalizeCalendar(obj) {
  // obj might be mapping from timestamp (string) to count, or date string to count
  const out = {};
  ...
  for (const k of Object.keys(obj)) {
    ...
    if (/^[0-9]{9,}$/.test(k)) {
      ...
      // attempt to parse as int timestamp
      const n = Number(k);
...
(async () => {
  try {
    ...
    // try to extract NEXT_DATA JSON first
    ...
    // fallback: look for window.__INITIAL_STATE__ = {...};
    if (!parsed) {
    ...
    // Also try to find 'submissionCalendar' or 'submitCalendar' recursively
    let calendar = null;
    ...
    // As a final fallback, try GraphQL endpoint
    if (!calendar || !acSubmissionNum) {
      ...
      try {
        ...
        if (mu) {
          ...
          // older responses might nest differently
          if (!acSubmissionNum) {
